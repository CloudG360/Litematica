...
public class BlockList {
    public List<WrappedItemStack> getList(final EntityPlayer player, final WorldSchematic world, final World mcWorld) {
        ...
        for (final MBlockPos pos : BlockPosHelper.getAllInBox(BlockPos.ORIGIN, new BlockPos(world.getWidth() - 1, world.getHeight() - 1, world.getLength() - 1))) {
            ...
            try {
                ...
                Litematica.logger.warn("Could not get the pick block for: {}", blockState, e);
            }
            ...
            if (stack == null) {
                Litematica.logger.error("Could not find the item for: {} (getPickBlock() returned null, this is a bug)", blockState);
                continue;
            }

            if (stack.isEmpty()) {
                Litematica.logger.warn("Could not find the item for: {}", blockState);
                continue;
            }
